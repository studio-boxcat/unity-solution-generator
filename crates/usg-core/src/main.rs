...
fn main() -> ExitCode {
    init_tracing();
    // Cache the watchman socket path once so subsequent Connector::connect()
    // calls skip the per-call `watchman get-sockname` subprocess (~120 ms each).
    unity_solution_generator::scan::init_socket_env();
