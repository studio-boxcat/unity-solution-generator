...
//! cdylib also ships as `rlib`). Pins the post-trim Rider call pattern:
//! `usg_generate(root, "ios", "editor", ".", extraRefs)`.
//!
...
/// Pinned: the exact call pattern Rider uses post-trim. Output buffer args
/// were dropped — Rider always passed `IntPtr.Zero, 0` and never read the path.
#[test]
